...
@pytest.mark.parametrize(
    "folder,expected",
    [
        (Path("tests/fixtures/collection"), True),
        (Path("tests/fixtures/expand_collection"), True),
        (Path("tests/fixtures/expand_collection_newlines"), True),
        (Path("tests/fixtures/has_deps"), True),
        (Path("tests/fixtures/not_collection"), True),
        (Path("tests/fixtures/nothing"), False),
    ],
)
def test_enabled(mocker, folder, expected):
    c = ConfigParser()
